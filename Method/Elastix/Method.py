...
class Method(Base):
    ...
    def SetMethodSettings(self, settings):
        ...
        self.__dataset = settings["dataset"]
        self.__environment = settings["environment"]
        self.__extension = settings["extension"]
        
        rootEnvDict = self.__environment.GetRootEnvironmentDict()
        if "WaitFunction" in rootEnvDict:
            self.__WaitFunction = rootEnvDict["WaitFunction"] 
    ...
    def __UpdateEnvironmentDict(self, datasetIndex):
        expSize = len(self.__parameters[0].GetValues())
        envDict = self.__environment.GetEnvironmentDictForDataset(datasetIndex, expSize)
    ...
    def __RunRigidElas(self, abortIfExist = False):
        ...
        elasDict = {"-f": self.__dataset["fixedIm"]}
