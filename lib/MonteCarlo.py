...
class MonteCarlo(Elastix):
    ...
    """ 
    def run(self,sampleNum=0,clusterBatchLim=50,transformixOnCluster=False,nonRigOnCluster=False,rigOnCluster=False):
    ...
    def saveStdImage(self,fileName):
        try:
            self.saveItkImage(fileName,self.getStdImage(),True)
        except:
            if self.__verbose:
    ...
    def getStdImage(self):
        return [self.__stdImage,self.__ImageOrigin,self.__ImageSpacing]
    ...
    def __accumulateFirstAndSecondMoments(self,startIndex,endIndex):
        for index in range(startIndex,endIndex):
            ...
            Vol=self.loadItkImage(self["defFieldDir"]+"/deformationField.mhd")
