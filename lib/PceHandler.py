...
class PceHandler(Elastix):
    ...
    def getStd(self):
        ...
        for cnt in range(0, self.__sampleNum):
            ...
            self.getDeformationField()
        cmd = self["prePceCommands"]
        cmd += self["PCE_Exe"] + " -rootDir " + self[
            "RegMainDir"] + " -verbose -uncertainty 1_2-1-2-all" + " -sobol " + "-polOrderFile " + " -outDir " + self[
                   "RegMainDir"] + " -settingsFile " + self["PCE_ModelSetRunFile"] + " -runPCE "
        cmd += self["postPceCommands"]
        os.system(cmd)
        for cnt in range(0, self.__sampleNum):
            self["defFieldDir"] = self["RegMainDir"] + "/NonRigid/Transformix" + str(cnt)
    ...
    def generateWeightFile(self):
        cmd = self["prePceCommands"]
        cmd += self["PCE_Exe"] + " -settingsFile " + self["PCE_ModelSetRunFile"] + " -writePceWeights " + self[
            "Pce_WeightFile"]
        cmd += self["postPceCommands"]
        os.system(cmd)
    ...
    def loadParamsFromPreviousRun(self, fromPrevious=False):
        self.__loadFrompreviousRun = fromPrevious
>>>>>>> 29945b7c58d40b3585fcccd2c50f44ce613047f5
